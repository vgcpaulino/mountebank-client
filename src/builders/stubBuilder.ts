...
export class StubBuilder {
    ...
    response({ status, headers, body, decorate, repeat, copyFromHeader, copyFromPath }: IResponseBuilder) {
        this.responses.push(
            new ResponseBuilder({ status, headers, body, decorate, repeat, copyFromHeader, copyFromPath })
        );
        return this;
    }
